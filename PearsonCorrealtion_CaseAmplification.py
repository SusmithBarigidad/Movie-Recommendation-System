...
def predictPearsonCaseAmplification(test_id, train_list, inv_ratings):
    test_user = test_id
    ...
    for target in test_user.targets:
        if target in inv_ratings:
            ...
            for trainer_id in top_n:
                top_n[trainer_id] = abs(int(top_n[trainer_id]))**(2.5-1)
...
for key in data_test.keys():
    ...
    for testID in data_test[key]:
        result.append(predictPearsonCaseAmplification(testID, train_list, inverted_list))
    resultOutput(result,'pearsonCaseAmplification'+str(key))